...
public interface Sampler {
  ...
  /**
   * Returns a {@link Sampler} that samples if and only if one span link {@link Span} is sampled. If
   * there are no links, the Sampler uses the provided Sampler delegate to determine the sampling
   * decision.
   *
   ...
   *     exist.
   * @return a {@code Sampler} that samples if and only if at least one span link is sampled. If no
   *     span links exist, then it delegates the decision to the root's sample decision.
   */
