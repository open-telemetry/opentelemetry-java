...
public final class SdkTracerProvider implements TracerProvider, Closeable {
    ...
    SdkTracerProvider(Clock clock, IdGenerator idsGenerator, Resource resource, Supplier<SpanLimits> spanLimitsSupplier,
            Sampler sampler, List<SpanProcessor> spanProcessors) {
        this.sharedState = new TracerSharedState(clock, idsGenerator, resource, spanLimitsSupplier, sampler,
                spanProcessors);
        this.tracerSdkComponentRegistry = new ComponentRegistry<>(
    ...
    /**
     * Returns the {@link Resource} that are currently applied to created spans;
     */
    ...
    /**
     * Attempts to stop all the activity for {@link Tracer}s created by this provider. Calls
     * {@link SpanProcessor#shutdown()} for all registered {@link SpanProcessor}s.
     *
     ...
     *
     * @return a {@link CompletableResultCode} which is completed when all the span processors have been shut down.
     */
     ...
     * {@link CompletableResultCode} which is completed when the flush is finished.
     *
    ...
    public String toString() {
        return "SdkTracerProvider{" + "clock=" + sharedState.getClock() + ", idGenerator="
                + sharedState.getIdGenerator() + ", resource=" + sharedState.getResource() + ", spanLimitsSupplier="
                + sharedState.getSpanLimits() + ", sampler=" + sharedState.getSampler() + ", spanProcessor="
