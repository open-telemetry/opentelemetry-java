...
public final class Samplers {
   ...
   * range. We convert an incoming ratio into an upper bound on that value, such that we can just
   * compare the absolute value of the id and the bound to see if we are within the desired ratio
   * range. Using the low bits of the traceId also ensures that systems that only use 64 bit ID's
   * will also work with this sampler.
   */
  @AutoValue
  @Immutable
  abstract static class TraceIdRatioBased implements Sampler {
    ...
    static TraceIdRatioBased create(double ratio) {
      ...
      return new AutoValue_Samplers_TraceIdRatioBased(
          ratio,
          idUpperBound,
          SamplingResultImpl.createWithProbability(Decision.RECORD_AND_SAMPLED, ratio),
          SamplingResultImpl.createWithProbability(Decision.NOT_RECORD, ratio));
    }
