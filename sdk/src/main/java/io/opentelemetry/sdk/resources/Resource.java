...
public abstract class Resource {
  ...
  private static final Resource EMPTY = create(Collections.<String, ResourceValue>emptyMap());
   ...
   * @since 0.1.0
   */
  public abstract Map<String, ResourceValue> getLabels();
   ...
   * @since 0.1.0
   */
  public static Resource create(Map<String, ResourceValue> labels) {
  ...
  public Resource merge(@Nullable Resource other) {
    ...
    Map<String, ResourceValue> mergedLabelMap = new LinkedHashMap<>(other.getLabels());
    // Labels from resource overwrite labels from otherResource.
    for (Entry<String, ResourceValue> entry : this.getLabels().entrySet()) {
      mergedLabelMap.put(entry.getKey(), entry.getValue());
    }
    return new AutoValue_Resource(Collections.unmodifiableMap(mergedLabelMap));
  }

  private static void checkLabels(Map<String, ResourceValue> labels) {
    for (Entry<String, ResourceValue> entry : labels.entrySet()) {
      Utils.checkArgument(
