...
public final class CallbackRegistration {
   ...
   * SdkObservableMeasurement#setActiveReader(RegisteredReader, long)} before {@code runnable} is
   * called, and set to {@code null} afterwards.
   *
  ...
  public void invokeCallback(RegisteredReader reader, long epochNanos) {
    ...
    observableMeasurements.forEach(
        observableMeasurement -> observableMeasurement.setActiveReader(reader, epochNanos));
    try {
      callback.run();
    } catch (Throwable e) {
      propagateIfFatal(e);
