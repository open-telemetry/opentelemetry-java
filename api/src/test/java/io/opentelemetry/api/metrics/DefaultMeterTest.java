...
class DefaultMeterTest {
  ...
  void expectDefaultMeter() {
    ...
    assertThat(OpenTelemetry.getGlobalMeter("test")).isSameAs(Meter.noop());
    assertThat(OpenTelemetry.getGlobalMeter("test", "0.1.0")).isSameAs(Meter.noop());
  }

  @Test
  void expectDefaultMeterProvider() {
    assertThat(OpenTelemetry.getGlobalMeterProvider()).isSameAs(MeterProvider.noop());
