...
public class JsonCompareUtil {
  ...
  static Boolean containsExactly(JSONArray actualJsonArray, JSONObject exceptedJson) {
    List<String> resultList = new LinkedList<>();
    for (int i = 0; i < actualJsonArray.size(); i++) {
      if (verifyJsonObject(actualJsonArray.getString(i))
          && verifyJsonObject(exceptedJson.toString())) {
  ...
  static Boolean containsExactly(JSONArray actualJsonArray, JSONArray exceptedJsonArray) {
    List<String> resultList = new LinkedList<>();
    for (int i = 0; i < exceptedJsonArray.size(); i++) {
      if (verifyJsonObject(actualJsonArray.getString(i))
          && verifyJsonObject(exceptedJsonArray.getString(i))) {
        if (compareJsonObject(
            actualJsonArray.getJSONObject(i), exceptedJsonArray.getJSONObject(i))) {
          resultList.add(String.valueOf(i));
        }
      } else {
  ...
  static Boolean compareJsonObject(JSONObject actualJson, JSONObject exceptedJson) {
    List<String> resultList = new LinkedList<>();
  ...
  private static Boolean verifyJsonObject(String jsonString) {
