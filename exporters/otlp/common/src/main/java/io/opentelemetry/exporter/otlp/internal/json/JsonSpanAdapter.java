...
public final class JsonSpanAdapter {
    ...
    resourceAndLibraryMap.forEach(
        (resource, librarySpans) -> {
          ...
          librarySpans.forEach(
              (library, spans) -> {
                instrumentationLibrarySpan.add(buildInstrumentationLibrarySpan(library, spans));
              });
  ...
  static JSONObject toProtoSpan(SpanData spanData, ThreadLocalCache threadLocalCache) {
    ...
    builder.put(
        "dropped_events_count", spanData.getTotalRecordedEvents() - spanData.getEvents().size());
    ...
    builder.put(
        "dropped_links_count", spanData.getTotalRecordedLinks() - spanData.getLinks().size());
  ...
  static JSONObject toProtoSpanEvent(EventData event, ThreadLocalCache threadLocalCache) {
    ...
    builder.put("attributes", attributes);
    builder.put(
        "dropped_attributes_count", event.getTotalAttributeCount() - event.getAttributes().size());
    return builder;
  }
  ...
  private static JSONObject buildInstrumentationLibrarySpan(
    ...
    spansBuilder.put(
        "instrumentation_library", JsonCommonAdapter.toProtoInstrumentationLibrary(library));
