...
public abstract class Timestamp implements Comparable<Timestamp> {
  ...
  public static Timestamp create(long seconds, int nanos) {
    if (seconds < -TimeUtils.MAX_SECONDS) {
      throw new IllegalArgumentException(
          "'seconds' is less than minimum (" + -TimeUtils.MAX_SECONDS + "): " + seconds);
    }
    if (seconds > TimeUtils.MAX_SECONDS) {
      throw new IllegalArgumentException(
          "'seconds' is greater than maximum (" + TimeUtils.MAX_SECONDS + "): " + seconds);
    }
    if (nanos < 0) {
      throw new IllegalArgumentException("'nanos' is less than zero: " + nanos);
    }
    if (nanos > TimeUtils.MAX_NANOS) {
      throw new IllegalArgumentException(
          "'nanos' is greater than maximum (" + TimeUtils.MAX_NANOS + "): " + nanos);
    }
  ...
  public static Timestamp fromMillis(long epochMilli) {
    long secs = floorDiv(epochMilli, TimeUtils.MILLIS_PER_SECOND);
    int mos = (int) floorMod(epochMilli, TimeUtils.MILLIS_PER_SECOND);
    return create(secs, (int) (mos * TimeUtils.NANOS_PER_MILLI)); // Safe int * NANOS_PER_MILLI
  }
  ...
  public Duration subtractTimestamp(Timestamp timestamp) {
    ...
    if (durationSeconds < 0 && durationNanos > 0) {
      durationSeconds += 1;
      durationNanos = (int) (durationNanos - TimeUtils.NANOS_PER_SECOND);
  ...
  private Timestamp plus(long secondsToAdd, long nanosToAdd) {
    ...
    epochSec = TimeUtils.checkedAdd(epochSec, nanosToAdd / TimeUtils.NANOS_PER_SECOND);
    nanosToAdd = nanosToAdd % TimeUtils.NANOS_PER_SECOND;
  ...
  private static Timestamp ofEpochSecond(long epochSecond, long nanoAdjustment) {
    long secs =
        TimeUtils.checkedAdd(epochSecond, floorDiv(nanoAdjustment, TimeUtils.NANOS_PER_SECOND));
    int nos = (int) floorMod(nanoAdjustment, TimeUtils.NANOS_PER_SECOND);
