...
| otel.exporter.prometheus.add-resource-attributes-as-labels | OTEL_EXPORTER_PROMETHEUS_ADD_RESOURCE_ATTRIBUTES_AS_LABELS | If set, resource attributes will be added to each data point labels, excluding `service.name`, `service.namespace`, `service.instance.id`, `telemetry.sdk.language`, `telemetry.sdk.name` and `telemetry.sdk.version`. Default is false |
| otel.exporter.prometheus.add-resource-attributes-regexp    | OTEL_EXPORTER_PROMETHEUS_RESOURCE_ATTRIBUTES_LABELS        | The regular expression used for specifying which resource attribute to copy, if `add-resource-attributes-as-labels` is set. Default is `.*` which is all (the excluded attributes are excluded regardless).                             |
